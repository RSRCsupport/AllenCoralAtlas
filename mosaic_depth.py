...
"""
Problem: To mosaic planet depth or bottom reflectance quads for operational purposes in the Allen Coral Atlas. This script takes as arguments: 
...
Examples: 
					...
					Local/path/where/data/and/scripts/are > python mosaic_depth.py -h
					
					usage: mosaic_w.py [-h] [-inputshp INPUTSHP] [-outputDataType OUTPUTDATATYPE]
                   inputTxtFile
...
optional arguments:
  ...
  -outputDataType OUTPUTDATATYPE
					...
					2) create a mosaic from a list of rasters in the text file "depthMosaic.txt" (in the linux terminal 
					by doing ls *.tif > depthMosaic.txt), using a shapefile (depthSubset.shp) and 
					setting Float32 as the final data type. IMPORTANT: the name of the .txt file that contains
					the list of rasters will be the final name for the mosaic. In this example "depthMosaic.tif"
					
					Local/path/where/data/and/scripts/are > python mosaic_depth.py depthMosaic.txt -inputshapefile depthSubset.shp -outputDataType Float32
...
def makeMosaic(dataType,infile): ## make one function to depth
    """
    ...
    :param dataType: data type argument 
	:param infile: subsetted vrt
    :return: the actual .tif s mosaic
    """
    ...
    cmd = "gdal_translate -of 'GTiff' -co COMPRESS=LZW -b 1 -ot {} -co NUM_THREADS=ALL_CPUS -co BIGTIFF=YES --config GDAL_CACHEMAX 80000 {} {}".format(dataType,infile, outname2)
    os.system(cmd)
    return (outname2)
